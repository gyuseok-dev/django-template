...
def update_daily_statistics(sender, instance: ManualRecord, created, **_kwargs):
    ...
    visit_stats = (
        VisitChannelRecord.objects.filter(manual_record=instance)
        .values("channel")
        .annotate(count=Count("id"))
    )

    channel_counts = {stat["channel"]: stat["count"] for stat in visit_stats}
    # 누락된 channel은 0으로 채우기
...
def update_monthly_statistics_for_date(date, hospital):
    ...
    manual_records = ManualRecord._default_manager.filter(
        hospital=hospital, date__year=year, date__month=month
    )

    if not manual_records.exists():
        # 데이터가 없으면 통계 삭제
        MonthlyStatistics._default_manager.filter(
            hospital=hospital, year=year, month=month
        ).delete()
        return

    # 월별 통계 가져오기 또는 생성
    monthly_stat, _ = MonthlyStatistics._default_manager.get_or_create(
        hospital=hospital, year=year, month=month
    )
    ...
    # 일별 통계 집계 (병원별)
    daily_stats = DailyStatistics._default_manager.filter(
        hospital=hospital, manual_record__in=manual_records
    )

    # 매출 통계
    monthly_stat.total_revenue = sum(ds.total_revenue for ds in daily_stats)
    monthly_stat.avg_daily_revenue = (
        monthly_stat.total_revenue // monthly_stat.working_days
        if monthly_stat.working_days > 0
        else 0
    )

    # 환자 통계
    monthly_stat.total_new_patients = sum(
        ds.new_patient_count for ds in daily_stats
    )
    monthly_stat.total_revisit_patients = sum(
        ds.revisit_patient_count for ds in daily_stats
    )
    monthly_stat.total_ninety_patients = sum(
        ds.ninety_day_patient_count for ds in daily_stats
    )
    monthly_stat.total_patients = sum(
        ds.total_patient_count for ds in daily_stats
    )

    if monthly_stat.working_days > 0:
        monthly_stat.avg_new_patients = round(
            monthly_stat.total_new_patients / monthly_stat.working_days, 1
        )
        monthly_stat.avg_revisit_patients = round(
            monthly_stat.total_revisit_patients / monthly_stat.working_days, 1
        )
        monthly_stat.avg_ninety_patients = round(
            monthly_stat.total_ninety_patients / monthly_stat.working_days, 1
        )
        monthly_stat.avg_total_patients = round(
            monthly_stat.total_patients / monthly_stat.working_days, 1
        )
    ...
    monthly_stat.fee_per_case = (
        monthly_stat.total_revenue // monthly_stat.total_patients
        if monthly_stat.total_patients > 0
        else 0
    )
...
def update_global_statistics(hospital):
    ...
    # 1. 최고 일일 매출
    max_daily = (
        DailyStatistics._default_manager.filter(hospital=hospital)
    ...
    if max_daily:
        GlobalStatistics._default_manager.update_or_create(
            ...
            defaults={
                "value": {
                    "amount": max_daily.total_revenue,
                    "date": str(max_daily.manual_record.date),
                },
    ...
    min_daily = (
        DailyStatistics._default_manager.filter(
            hospital=hospital, total_revenue__gt=0
        )
        .order_by("total_revenue")
        .first()
    )
    if min_daily:
        GlobalStatistics._default_manager.update_or_create(
            ...
            defaults={
                "value": {
                    "amount": min_daily.total_revenue,
                    "date": str(min_daily.manual_record.date),
                },
    ...
    # 4. 최고 월간 매출
    max_monthly = (
        MonthlyStatistics._default_manager.filter(hospital=hospital)
        .order_by("-total_revenue")
        .first()
    )
    if max_monthly:
        GlobalStatistics._default_manager.update_or_create(
            ...
            defaults={
                "value": {
                    "amount": int(max_monthly.total_revenue),
                    "year": max_monthly.year,
                    "month": max_monthly.month,
                },
    ...
    if min_monthly:
        GlobalStatistics._default_manager.update_or_create(
            ...
            defaults={
                "value": {
                    "amount": int(min_monthly.total_revenue),
                    "year": min_monthly.year,
                    "month": min_monthly.month,
                },
