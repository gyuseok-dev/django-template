...
class ManualRecordMonthFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if value:
            ...
            return queryset.filter(manual_record__date__year=year, manual_record__date__month=month)
        return queryset
...
class PatientRecordResource(resources.ModelResource):
    ...
    visit_at = fields.Field(column_name="내원일", attribute="visit_at", widget=DateTimeWidget(format="%Y%m%d%H%M"))
    ...
    def before_import(self, dataset, **kwargs):
        ...
        """
        header_keywords = ["차트번호", "chart_number", "수진자명", "patient_name", "챠트번호"]
        ...
        if header_row_index is not None:
            # 헤더 행을 새로운 헤더로 설정
            dataset.headers = [str(cell).strip() if cell else "" for cell in dataset[header_row_index]]
            # 헤더 이전 행들 + 헤더 행 삭제
...
class VisitChannelRecordResource(resources.ModelResource):
    ...
    visit_at = fields.Field(column_name="내원날짜", attribute="visit_at", widget=DateTimeWidget(format="%Y%m%d%H%M"))
...
class PatientRecordAdmin(TenantAdminMixin, ModelAdmin, ImportExportModelAdmin):
    ...
    # Admin 인터페이스 사용자 정의
    fieldsets = (
        ("기본 정보", {"fields": ("chart_number", "patient_name", "patient_id", "get_masked_patient_id")}),
        ("진료 정보", {"fields": ("room", "insurance_type", "visit_type", "doctor_name", "visit_at")}),
        (
            "메타 정보",
            {
...
class VisitChannelRecordAdmin(TenantAdminMixin, ModelAdmin, ImportExportModelAdmin):
    """내원 경로 Admin"""
    ...
    fieldsets = (
        ...
        ("진료 정보", {"fields": ("room", "insurance_type", "visit_type", "doctor_name")}),
        (
            "메타 정보",
            {
                "fields": ("created_at", "updated_at"),
...
class ManualRecordAdmin(TenantAdminMixin, ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        class FormWithHospitalId(form_class):
    ...
    @admin.display(description="총진료비")
    def get_total_jinryobi(self, obj):
        """총진료비 표시"""
        return f"{obj.total_jinryobi:,}원"

    @admin.display(description="총매출")
    def get_total_maechul(self, obj):
        """총매출 표시"""
        return f"{obj.total_revenue:,}원"
    ...
    def save_model(self, request, obj, form, change):
        ...
        with transaction.atomic():
            ...
            # medical_record_file 처리
            if obj.medical_record_file and hasattr(obj.medical_record_file, "file"):
                self._import_medical_records(request, obj)

            # visit_source_file 처리
            if obj.visit_source_file and hasattr(obj.visit_source_file, "file"):
                self._import_visit_channel_records(request, obj)

    def _import_medical_records(self, request, obj):
        """환자 진료 기록 파일 import"""
        ...
        try:
            ...
            if file_name.endswith(".xlsx"):
                ...
                self.message_user(
                    request, "지원하지 않는 파일 형식입니다. .xlsx 또는 .xls 파일을 업로드해주세요.", level="error"
                )
                return

            # PatientRecordResource를 사용해서 import (manual_record를 전달하여 import 시점에 연결)
            resource = PatientRecordResource()
            result = resource.import_data(dataset, dry_run=False, raise_errors=True, manual_record=obj)

            self.message_user(
                request,
                f"환자 진료 기록 import 완료: 신규 {result.totals['new']}건, 수정 {result.totals['update']}건, "
                f"삭제 {result.totals['delete']}건, 건너뜀 {result.totals['skip']}건",
                level="success",
            )
    ...
    def _import_visit_channel_records(self, request, obj):
        """내원 경로 파일 import"""
        from tablib import Dataset
        ...
        try:
            ...
            # VisitChannelRecordResource를 사용해서 import (manual_record를 전달하여 import 시점에 연결)
            resource = VisitChannelRecordResource()
            result = resource.import_data(dataset, dry_run=False, raise_errors=True, manual_record=obj)

            self.message_user(
                request,
                f"내원 경로 import 완료: 신규 {result.totals['new']}건, 수정 {result.totals['update']}건, "
                f"삭제 {result.totals['delete']}건, 건너뜀 {result.totals['skip']}건",
                level="success",
            )
    ...
    def delete_model(self, request, obj):
        ...
        - 월별/전역 통계 업데이트
        """
        from record.signals import update_global_statistics, update_monthly_statistics_for_date
    ...
    def delete_queryset(self, request, queryset):
        ...
        for year, month in months_updated:
            ...
            update_monthly_statistics_for_date(date(year, month, 1), queryset.first().hospital)
    ...
    def recalculate_statistics(self, request, queryset):
        ...
        for manual_record in queryset:
            # 일별 통계 재계산
            daily_stat, _ = DailyStatistics.objects.get_or_create(manual_record=manual_record)

            # 총매출 계산
            daily_stat.total_revenue = manual_record.total_revenue

            # 환자 통계 계산
            patient_records = PatientRecord.objects.filter(manual_record=manual_record)
            ...
            # 내원 경로 통계
            visit_records = VisitChannelRecord.objects.filter(manual_record=manual_record)
            channel_counts = {}
            ...
            # 해당 월 추적
            months_updated.add((manual_record.date.year, manual_record.date.month))
            count += 1
